    ...
    gameState._seabedDecor.forEach((decor) => {
        ...
        if (decor.type === "kelp") {
            ...
            ctx.quadraticCurveTo(
                screenX + swayAmount * 1.5,
                baseY - decor.height * 0.5,
                screenX + swayAmount * 2.5,
                baseY - decor.height
            );
            ctx.stroke();

            for (let l = 0; l < 3; l++) {
                const leafY = baseY - decor.height * (0.3 + l * 0.25);
                const leafDir = l % 2 === 0 ? 1 : -1;
                ctx.strokeStyle = `hsla(${decor.hue}, 50%, 40%, ${decor.alpha * 0.7})`;
                ctx.lineWidth = 2;
                ctx.beginPath();
                ctx.moveTo(screenX + swayAmount * (0.6 + l * 0.5), leafY);
                ctx.quadraticCurveTo(
                    screenX + swayAmount * (0.6 + l * 0.5) + leafDir * 10,
                    leafY - 5,
                    screenX + swayAmount * (0.6 + l * 0.5) + leafDir * 14,
                    leafY + 2
                );
                ctx.stroke();
            }
            ...
            ctx.beginPath();
            ctx.arc(screenX, baseY - 4, decor.width * 0.6, Math.PI, 0);
            ctx.fill();

            ctx.fillStyle = `hsla(${decor.hue + 15}, 50%, 55%, ${decor.alpha * 0.6})`;
            ctx.beginPath();
            ctx.arc(screenX + decor.width * 0.3, baseY - 6, decor.width * 0.4, Math.PI, 0);
            ctx.fill();

            ctx.fillStyle = `hsla(${decor.hue - 10}, 45%, 50%, ${decor.alpha * 0.5})`;
            ctx.beginPath();
            ctx.arc(screenX - decor.width * 0.25, baseY - 3, decor.width * 0.35, Math.PI, 0);
            ctx.fill();
            ctx.restore();
        }
    });
...
function drawSubmarine() {
    ...
    const glassGrad = ctx.createRadialGradient(
        submarine.width / 2 + 6, submarine.height / 2 - 2, 1,
        submarine.width / 2 + 8, submarine.height / 2, 7
    );
...
function drawFish(fishEntry) {
    ...
    if (centerX < -50 || centerX > canvas.width + 50 || centerY < -50 || centerY > canvas.height + 50) {
        return;
    }

    // Sonar ring animation for nearby
    if (nearby) {
        ...
        for (let ring = 0; ring < 2; ring++) {
            ...
            ctx.beginPath();
            ctx.arc(centerX, centerY + bob, ringRadius, 0, Math.PI * 2);
    ...
    if (nearby) {
        glowGrad.addColorStop(0, "rgba(94, 228, 255, 0.15)");
        glowGrad.addColorStop(1, "rgba(94, 228, 255, 0)");
    } else {
        glowGrad.addColorStop(0, "rgba(255, 255, 255, 0.05)");
        glowGrad.addColorStop(1, "rgba(255, 255, 255, 0)");
    }
    ctx.beginPath();
    ctx.arc(centerX, centerY + bob, glowRadius, 0, Math.PI * 2);
    ctx.fillStyle = glowGrad;
    ctx.fill();

    // Main circle with gradient
    const circleGrad = ctx.createRadialGradient(
        centerX - 3, centerY + bob - 3, 2,
        centerX, centerY + bob, fishEntry.radius
    );
    if (nearby) {
        circleGrad.addColorStop(0, "#1a4860");
        circleGrad.addColorStop(1, "#0a2535");
    } else {
        circleGrad.addColorStop(0, "#405e6e");
        circleGrad.addColorStop(1, "#2a4050");
    }
    ctx.beginPath();
    ctx.arc(centerX, centerY + bob, fishEntry.radius, 0, Math.PI * 2);
